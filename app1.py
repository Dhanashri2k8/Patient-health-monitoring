...
# Precautions for each vital if out of range
precautions = {
...
if "user" in st.session_state:
    ...
    if st.button("Predict Risk"):
        ...
        # --- Precaution Messages ---
        precautions = {
            "Heart Rate High": "Avoid caffeine, stress, and consult a doctor if persistent.",
            "Heart Rate Low": "Check for dizziness, avoid sudden standing, consult doctor if frequent.",
            "Respiratory Rate High": "Practice deep breathing, check for asthma or infection, consult doctor.",
            "Respiratory Rate Low": "Check for fatigue, consult doctor if breathing difficulty occurs.",
            "Body Temperature High": "Drink fluids, rest, use a cold compress, seek medical help if above 39°C.",
            "Body Temperature Low": "Keep warm, drink warm fluids, consult doctor if very low (<35°C).",
            "Oxygen Saturation Low": "Sit upright, practice deep breathing, check oxygen supply, consult doctor.",
            "Systolic Blood Pressure High": "Reduce salt intake, manage stress, take medicines if prescribed.",
            "Systolic Blood Pressure Low": "Stay hydrated, increase salt (if allowed), consult doctor if dizziness persists.",
            "Diastolic Blood Pressure High": "Reduce stress, lower salt intake, follow doctor’s treatment plan.",
            "Diastolic Blood Pressure Low": "Stay hydrated, avoid alcohol, consult doctor if feeling weak.",
            "Derived_BMI High": "Maintain diet & exercise, consult a nutritionist.",
            "Derived_BMI Low": "Improve nutrition, eat balanced diet, consult doctor if underweight.",
            "Derived_MAP High": "Manage blood pressure, follow doctor’s advice.",
            "Derived_MAP Low": "Monitor hydration, consult doctor immediately if very low."
        }

        # --- Check Abnormal Vitals + Precautions ---
        abnormal_vitals = []
        precaution_msgs = []
        ...
        for vital, (low, high) in normal_ranges.items():
            ...
            if isinstance(value, (int, float)):
                if value < low:
                    abnormal_vitals.append(f"{vital}: {value} (Low)")
                    if f"{vital} Low" in precautions:
                        precaution_msgs.append(f"💡 {vital}: {precautions[f'{vital} Low']}")
                elif value > high:
                    abnormal_vitals.append(f"{vital}: {value} (High)")
                    if f"{vital} High" in precautions:
                        precaution_msgs.append(f"💡 {vital}: {precautions[f'{vital} High']}")
        ...
        if pred == 1:
            ...
            if abnormal_vitals:
                st.warning("Some vitals are outside normal range (not critical):")
                for vital in abnormal_vitals:
                    st.write(f"🟠 {vital}")
                st.info("### Recommended Precautions:")
                for p in precaution_msgs:
        ...
        row = {
            ...
            **patient_data,
            ...
            "Precautions": " | ".join(precaution_msgs) if precaution_msgs else "None"
        }
